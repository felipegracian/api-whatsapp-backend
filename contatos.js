...
const getDadoscontato = (numeroContato) => {
  let jsonWhatsUser = {}
  let arrayDadosContato = []
  let arrayContacts = []
  let status = false

  contatos["whats-users"].forEach(function (pegarNumero) {
    if (pegarNumero.number == numeroContato) {
      let jsonComOsDadosDoContato = {}
      jsonComOsDadosDoContato.account = pegarNumero.account
      jsonComOsDadosDoContato.nickname = pegarNumero.nickname
      jsonComOsDadosDoContato.number = pegarNumero.number
      jsonComOsDadosDoContato.profile_image = pegarNumero["profile-image"]
      arrayDadosContato.push(jsonComOsDadosDoContato)



      pegarNumero.contacts.forEach(function (pegarContatosDoUsuario) {



        pegarContatosDoUsuario.contacts = arrayContacts
        let jsonComContatosDoUsuario = {}
        jsonComContatosDoUsuario.name = pegarContatosDoUsuario.name
        jsonComContatosDoUsuario.description = pegarContatosDoUsuario.description
        jsonComContatosDoUsuario.image = pegarContatosDoUsuario.image


        jsonComContatosDoUsuario.messages = pegarContatosDoUsuario.messages
        pegarContatosDoUsuario.messages.forEach(function (pegarInformacoesDeCadaMensagem) {
          let jsonMessages = {}
          jsonMessages.sender = pegarInformacoesDeCadaMensagem.sender
          jsonMessages.contente = pegarInformacoesDeCadaMensagem.content
          jsonComContatosDoUsuario.messages.push(jsonMessages)
        })




        arrayContacts.push(jsonComContatosDoUsuario)
  ...
  if (status == true) {
    jsonWhatsUser.whats_user = arrayDadosContato
    jsonWhatsUser.contacts = arrayContacts

    return jsonWhatsUser
  } else{
    return status
  }
